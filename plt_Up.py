...
unique_up_values = np.unique(Fp_matrix[:, 1])

for up_value in unique_up_values:
    # Filter data for each unique Up value
    filtered_data = Fp_matrix[Fp_matrix[:, 1] == up_value]

    Ompy_values = filtered_data[:, 0]
    Fp_z_values = filtered_data[:, 2]
    Tp_y_values = filtered_data[:, 3]

    # Plot
    plt.figure()
    plt.plot(Ompy_values, Fp_z_values, marker='o', color='blue', label='Fp_z')
    plt.plot(Ompy_values, Tp_y_values, marker='o', color='green', label='Tp_y')
    plt.xlabel('Ompy')
    plt.ylabel('Value')
    plt.title('Fp_z and Tp_y as a function of Ompy (Up = {})'.format(up_value))
    plt.legend()
    plt.grid(True)
    plt.show()
